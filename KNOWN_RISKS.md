...
escrowed shares are worth at the post-loss price. A queued lender whose floor is not fully
serviceable is held until a repayment or another holder's cancel, with no clock on either.
**There is no guaranteed recovery time**: both releases are somebody else's act, a borrower's
repayment or another holder's cancel, and neither is owed on any schedule. She can leave the lock
at any time by cancelling, at the cost of releasing her floor to the others.
...
reopens each door to 29.999999.
...
separately from the original High as #61. The maintainer's disposition is posted on
...
corrected and multiple-request coverage added. Both are in this commit. The recovery conditions
are the two named above, a repayment or a floor holder's cancel, and there is no guaranteed time
at which either arrives.
...
above states it present tense). The reviewers answered on 2026-09-17 (comment 5718920756): record the residual separately as a Low, acknowledged and retained by design, and close the original High without the write-down once the stated bound is corrected, which this sync does. The audit is not complete. Those four are the verification comments as
