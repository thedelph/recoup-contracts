...
### 33audits H-03, the post-loss lock the floor retains. Low, disclosed and held, dated 2026-09-17, pending the reviewers' disposition on #47
...
loss. After a raw loss the cash locked is max(0, floors owed minus executable cash):
`_queueCashReserve` holds the larger of the pro-rata fraction and the floors, clamped at the
executable cash, so the whole of that cash is reserved while the floors exceed it. The locked
amount is bounded above by the raw loss, because the floors never exceeded the cash at the moment
each was quoted (`_freshFloor` prices a floor out of the cash not already reserved for the requests
ahead of it), so only cash destroyed can put their sum above the cash; and it is bounded by
`_floorTotal` at the moment of loss, which no later request can raise above the cash, because a
request filed in the locked state is quoted a floor of 0.
...
repayment lifts the executable cash above the floors or a floor holder cancels. When a holder
...
other request. A queued lender whose floor is not fully serviceable is held until a repayment or
another holder's cancel, with no clock on either; she can leave the lock at any time by cancelling,
at the cost of releasing her floor to the others. There is no privileged path: `claimSolvencyDeficit()`
...
same 500.000002 independently on 68c0c26 on 2026-09-16, cancel included.
...
function. A lender who wants a reservation queues for one, and a queued lender who wants out of the
lock cancels, which releases her floor to the other live request.

**Status.** Low. Disclosed and held, dated 2026-09-17; the maintainer's disposition is posted on #47
as comment 5715504360, and the reviewers' disposition is pending and will be recorded here when it
lands.
...
above states it present tense) and awaits the reviewers' disposition; the audit is not complete. Those four are the verification comments as
of 2026-09-17; the other six rows stand on this record's own tests.
