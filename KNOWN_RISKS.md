...
core protocol executable logic in this repository is current as of 2026-09-14, and everything
...
external auditors; where the 2026-09-12 and 2026-09-14 syncs change what a sentence in it says,
...
external reviewers filed against b66023d, as it stands after the 2026-09-14 sync.
...
### The withdrawal-request "reserve" is a per-call pro-rata bound, not a reservation. Documentation, corrected in this source since 2026-09-12; the request door is bounded per controller since 2026-09-14
...
per-call bound is the intended semantics of the reserve, and a checkpointed cash floor is the
refused F7 shape. What was wrong at b66023d was the wording, and the previous revision of this
...
total equals the single slice. What that does not make the reserve is a cash guarantee. It is a
fraction of live executable cash, re-derived on every read, so the synchronous door stepped, or a
position walked through fresh controllers, still reaches the total the loop reached, and a queued
lender's serviceable figure falls with each step; the #47 row carries the figures. The file header,
the `maxRequestRedeem` docstring and the `queueCashReserve` docstring in `LenderPool` say all of
this; at commit b66023d they overstated the bound as a reservation, and at the 2026-09-12 sync they
understated what the stepped loop could reach.
...
below is the status of each issue **in this source from the 2026-09-14 sync on**; the 2026-09-12
sync was the first since the issues were filed, and the 2026-09-14 sync adds the M-04 ceiling, the
L-02 fix and the H-03 request-draw memory. Every function and test it names is in `src/` or `test/`
...
| #47 H-03 | Repeated request service converts more than a requester's pro-rata cash into senior claims | **Fixed on the request door with the reviewers' shape; High stays, because the memory closes one route of three to the same cash.** The 2026-09-12 revision of this row said the stepped loop reaches no more than the synchronous door already allows; that was measured with nobody else queued and is false with another lender's request holding the reserve. Counter-case, two lenders of 10,000 with 15,000 lent and the other lender's whole position queued: the same-controller request loop paid 4,999.999998 over 53 calls against 2,500.000000 from one synchronous `redeem` of the same shares, 45 of 60 cells of a grid over the queue and the loan favoured the loop, and the best excess was 3,999.999995 (`test_R59A02_H03_aSecondLendersQueueMakesTheLoopBeatTheSyncDoor` and `test_R59A02_H03_gridSearchForTheBestCounterCase` in [`test/R59A02_H03CounterCase.t.sol`](test/R59A02_H03CounterCase.t.sol), which now assert the closed state and keep those figures as history). The fix is the reviewers' per-controller request-draw memory (`RequestDraw`, `_requestDraws`, declared last so no existing slot moves): `maxRequestRedeem` adds the controller's earlier draws back into all three terms of the slice and deducts them from the answer, and `serviceWithdrawalRequest` records each service and clears the memory only once the request is empty and the controller holds no shares, which is what closes cancel-and-re-request. The reviewers' eight tests pass verbatim in [`test/R60S1_H03RequestDraw.t.sol`](test/R60S1_H03RequestDraw.t.sol): the loop 2,500.000000 in one call, the queued lender keeping 1,428.571428, 0 of 60 grid cells, cancel-and-re-request 2,500.000000; and `test_H03_steppedRequestServiceReachesNoMoreThanOneSyncRedeem` in [`test/Impairment.integration.t.sol`](test/Impairment.integration.t.sol) now asserts the stepped total equals the single slice, 4,874.250000. What it does not close, measured on this source in [`test/R60S1_H03Routes.t.sol`](test/R60S1_H03Routes.t.sol): `queueCashReserve` is the ceiling of executable cash times `queuedShares` over supply, a fraction of live cash rather than an amount of it, and every exit lowers the cash the fraction is taken of, so stepping the synchronous door, `redeem(maxRedeem)` until it reads zero, pays 4,999.999998 over 53 calls from the same state with no request serviced and the memory never consulted, and walking the position through forty fresh controllers one request each pays 4,999.999773 through either door; the queued lender's serviceable figure ends at 0.000001 either way. Two side effects of the memory, both measured in [`test/R60S1_H03Probes.t.sol`](test/R60S1_H03Probes.t.sol): after the one permitted draw the reserve still counts the drawn controller's remaining queued shares, so 2,500.000000 stays reserved against 1,428.571428 serviceable and 1,071.428572 of cash is reachable by no door, not the queued lender, not a third lender's `maxRedeem`, not `lend`, until cash or supply moves (a full repayment closes it); and the memory outlives the position it priced, surviving `cancelWithdrawalRequest`, a whole-position synchronous exit and a re-deposit, so a controller whose ratio of cash to supply has fallen since a draw is paid less than a fresh address holding the same shares (0 against 750.000000; 750.000000 against 2,166.666666 after a re-deposit). Neither is a route to more cash; both are lender-hostile in a way the previous code was not. Never clearing the memory (+172 bytes, strictly worse on the second point), clearing it on transfer (reopens the loop with a one-wei self-transfer), clearing it on request (reopens it through cancel) and an aggregate reconstruction of the reserve over live requests (+532, measures identically on every probe; +533 for the form that also re-derives every controller's entitlement from the aggregate, which closes the phantom but over-promises 1,999.999999 against 1,500.000000 executable after a raw cash loss between draws) were costed and refused. `invariant_oneRequestNeverDrawsMoreThanOneSliceOfTheLiveCash` in [`test/LenderPool.invariants.t.sol`](test/LenderPool.invariants.t.sol) holds a request's cumulative draw to the entitlement reconstructed over it, mirrored in the handler because a first form that read the pool's own memory back stayed green against a pool neutered to forget half of it. The file header, the `queueCashReserve` docstring and the `maxRequestRedeem` docstring name the counter-case, the two open routes, the phantom reserve and the stale memory. The design question the two open routes leave is open with the reviewers on the issue: either pro-rata-of-live-cash is the design and a queued lender's reserve is a fraction and not a floor, in which case those routes are the design working, or a queued lender is owed a cash amount, which is the re-derivable floor refused twice before for freezing the entitlement | `LenderPool` +219 |
...
Measured on this source with `forge build --sizes` on a clean build on 2026-09-14: `CreditManager`
22,461 bytes of runtime with 2,115 of EIP-170 margin and 23,796 of initcode with 25,356 of EIP-3860
margin; `LiquidationAuction` 21,022 / 3,554 and 22,437 / 26,715; `LenderPool` 20,856 / 3,720 and
22,228 / 26,924. `CreditManager` is the contract that binds. Against the 2026-09-12 sync the
manager is 57 bytes of runtime smaller (L-02), the pool carries M-04's +18 and H-03's +219, and the
auction is unmoved.

Of the six audited files, the 2026-09-14 sync changes `src/LenderPool.sol` (the M-04 clamp and
the H-03 memory, on top of the L-01, M-05 and H-03 docstring changes of the 2026-09-12 sync) and
`src/Config.sol` (one constant, `MAX_YIELD_STREAM_DURATION`, with its docstring; otherwise
byte-identical to b66023d). `src/LenderPool.sol` additionally differs from b66023d by the
...
This repository is a curated publication of the protocol's contracts, current as of 2026-09-14.
...
and the per-controller request-draw memory (`_requestDraws`, H-03).
