...
///
///      Usage:
///        forge script script/DeployReferral.s.sol:DeployReferralRegistry --rpc-url <url> --broadcast
///
...
contract DeployReferralRegistry is Script {
    ...
    ///      partner rule below forbids. If DexFi ever wants it, they register it themselves, or
    ///      `registerFor` claims it directly to their payout address.
    ///
    ...
    function run() external returns (ReferralRegistry registry) {
        // A stray `forge script` should not be able to reach a public chain by accident. Local
        // runs need no ceremony.
        ...
        console.log("Partner codes: registerFor(code, partnerPayoutAddress), BEFORE any announcement.");
    }
}
